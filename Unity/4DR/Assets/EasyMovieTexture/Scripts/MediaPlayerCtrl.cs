...
public class MediaPlayerCtrl : MonoBehaviour
{
    ...
    void Update()
    {
		...
		//Debug.Log("Unity :::: " + m_CurrentState);
	...
	private AndroidJavaObject GetJavaObject()
	{
		if (javaObj == null)
		{
			...
			//javaObj = new AndroidJavaObject("com.fdreplay.fdp.EasyFDPlayer");	
		}
	...
	private void Call_Left(bool _how)
	{	
		GetJavaObject().Call("_LEFT", new object[] { _how });
	}
	...
	private unsafe bool Call_Load(string strFileName, int iSeek)
	{
		...
		if (ffmpeg.avformat_find_stream_info(pFormatContext, null) != 0)
		{
			...
			//string tmp = Encoding.UTF8.GetString(pFormatContext->rtsp_session_id, 256);

			//Debug.Log("tmp :: " + tmp);
			Debug.Log("#### Start");
			//Debug.Log("#### f : " + new string(pFormatContext->filename));
			//Debug.Log("#### s : " + new string(pFormatContext->rtsp_session_id));
			//Debug.Log("#### int : " + (int)pFormatContext->rtsp_session_id);
			//Debug.Log("#### : " + int.Parse(pFormatContext->rtsp_session_id));
			Debug.Log("#### End");
				...
				if (pPacket->stream_index == iStreamIndex) {
					...
					if (size < 0) {
						...
						if (gotPicture == 1) {
							...
							//Debug.Log("pts :: " + pts + "/" + pPacket->duration);
